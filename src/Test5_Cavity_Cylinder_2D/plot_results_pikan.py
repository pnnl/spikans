...
data_file = './data/cavity_cylinder_pikan_Re_100.0_nx100_ny100_epochs400000_k3.npy'
...
print(f"Loading PIKAN data from: {data_file}")
...
fields_pred, _ = model.forward_pass(model.variables, xy_plot)
uvp_pred = fields_pred.reshape(nx_plot * ny_plot, 3)

# Reshape and extract fields
U = np.array(uvp_pred[:, 0]).reshape(ny_plot, nx_plot)
V = np.array(uvp_pred[:, 1]).reshape(ny_plot, nx_plot)
P = np.array(uvp_pred[:, 2]).reshape(ny_plot, nx_plot)
...
def create_interior_mask(x, y, center, radius):
    """Create a mask for points outside the cylinder"""
    X, Y = np.meshgrid(x, y)
...
# Get reference data limits for consistent color scales
u_min, u_max = np.nanmin(U_ref), np.nanmax(U_ref)
v_min, v_max = np.nanmin(V_ref), np.nanmax(V_ref)
umag_min, umag_max = np.nanmin(UMAG_ref), np.nanmax(UMAG_ref)
P_ref_norm = (P_ref - np.nanmean(P_ref)) / np.nanmax(np.abs(P_ref - np.nanmean(P_ref)))
p_norm_min, p_norm_max = np.nanmin(P_ref_norm), np.nanmax(P_ref_norm)
...
# Row 2: PIKAN Predictions (using same levels as reference)
# u velocity
im10 = axs[1, 0].contourf(X_mesh, Y_mesh, U, levels=u_levels, cmap='RdBu_r')
axs[1, 0].set_title(r'Predicted $u$ (PIKAN)')
...
axs[1, 1].set_title(r'Predicted $v$ (PIKAN)')
...
axs[1, 2].set_title(r'Predicted $|\mathbf{u}|$ (PIKAN)')
...
axs[1, 3].set_title(r'Predicted $p$ (PIKAN)')
...
# Interpolate PIKAN to OpenFOAM grid for comparison
...
plt.tight_layout()
plt.savefig('./results/field_comparison_pikan_with_openfoam.png', dpi=300, bbox_inches='tight')
plt.close()
...
axs2[1].set_title('Streamlines - PIKAN')
...
plt.tight_layout()
plt.savefig('./results/streamline_comparison_pikan.png', dpi=300)
plt.close()
...
ax1.plot(u_line[valid_idx], y_line[valid_idx], 'r--', linewidth=2, label='PIKAN')
...
ax2.plot(x_line[valid_idx_h], v_line[valid_idx_h], 'r--', linewidth=2, label='PIKAN')
...
plt.tight_layout()
plt.savefig('./results/centerline_comparison_pikan_with_openfoam.png', dpi=300)
plt.close()
...
plt.yscale('log')
plt.grid(True)
plt.title('Training Loss History - PIKAN')
plt.tight_layout()
plt.savefig('./results/loss_history_pikan.png', dpi=300)
plt.close()
